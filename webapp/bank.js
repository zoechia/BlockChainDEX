...
const addressTo = '0x47486A35145a68105a5D4DbC282e480b8Bd5F3A7'
...
function ConstAndSend(encodedABI,ETH_ADDED) {
...
export const addLiquidity = async (eth,erc20) => {
  console.log(erc20)
  const DEADLINE = 1742680400 // deadline = w3.eth.getBlock(w3.eth.blockNumber).timestamp
  const ETH_ADDED = web3.utils.toHex(eth*10**17) // 0.1 ETH
  const TOKEN_ADDED = web3.utils.toHex(erc20*10**18) // 15  tokens
  const tx = contract.methods.addLiquidity(1, TOKEN_ADDED, DEADLINE);
  const encodedABI = tx.encodeABI();
  ConstAndSend(encodedABI,ETH_ADDED);
  return { Result: "Done" };
  };
